...
class Page:
    ...
    def page_list(self):
        ...
        if mid_start >= 3:
            ...
            left = []
        if mid_end <= last - 1:
            right = range(last - 1, last + 1)
        else:
            right = []
        if len(left) == 2 and mid_start - left[1] > 1:
            left_flag = True
        if len(right) and right[0] - mid_end > 0:
            right_flag = True
        return [left, left_flag, range(mid_start, mid_end), right_flag, right]
